...
gulp.task('clean', function() {
    return gulp.src(['dist', 'lib-instrumented', 'test/coverage'], {
            read: false
        })
        .pipe(clean());
});
...
gulp.task('compile', ['clean'], function() {
    return browserify('./lib/bespoke-advanced.js')
        .exclude('bespoke-convenient')
        .bundle({
            standalone: 'bespoke.plugins.advanced'
        })
        .pipe(source('bespoke-advanced.js'))
        .pipe(buffer())
        ...
        .pipe(gulp.dest('dist'))
        .pipe(rename('bespoke-advanced.min.js'))
        .pipe(uglify())
        .pipe(header(template([
            '/*! <%= name %> v<%= version %> ',
